...
const V21_PAGE_SIZES=[30,60,90];
const V21_AI_PROMPT_GOALS={
 bot_description:'商品情報から、特徴・利用シーン・購入メリットが正確に伝わる商品説明文を提案する',
 bot_title:'商品情報から、誤解を招かず魅力が伝わる商品名候補を提案する',
 bot_tags:'商品情報と検索意図から、関連性の高い検索タグを提案する',
 bot_seo:'検索意図と商品情報を両立したSEOタイトルを提案する',
 bot_price:'販売価格・製造原価・手数料を踏まえ、根拠付きの価格候補を提案する',
 bot_analysis:'閲覧・お気に入り・購入等の集計値から、販売改善案を提案する',
 bot_ads:'商品の事実情報だけを使い、媒体に合った短い広告コピーを提案する',
 bot_customer:'問い合わせ内容を整理し、丁寧で安全な返信案を提案する',
 bot_review:'審査結果と差し戻し理由を整理し、具体的な修正案を提案する',
 bot_trend:'需要と販売傾向を整理し、断定を避けながら商品企画の仮説を提案する',
 bot_idea:'未充足需要やカテゴリ傾向から、実現可能な商品企画案を提案する',
 bot_market:'検索需要と供給の差を整理し、市場機会の仮説を提案する',
 bot_improve:'閲覧・お気に入り・購入率から、優先度付きの商品改善案を提案する'
};
function v21DefaultBotPrompt(bot={}){const goal=V21_AI_PROMPT_GOALS[bot.id]||bot.desc||'[このボットの目的を具体的に記載してください]';return `あなたはSticker Marketの「${bot.name||'運営支援AI'}」です。\n\n目的:\n- ${goal}\n\n出力要件:\n- 結論を最初に示す\n- 提案理由と具体的な次の操作を分けて示す\n- 必要に応じて複数案を提示する\n- 日本語で簡潔に回答する\n\n安全・品質:\n- 入力にない商品情報、実績、権利情報を作らない\n- 個人情報や決済情報を出力しない\n- 著作権・商標権を侵害する提案をしない\n- AIの提案は下書きとし、公開や変更を自動実行しない`;}
...
searchPage=function(){
 ...
 list=list.map(x=>x[0]);
 const requestedPageSize=Number(sp.get('pageSize')||V21_PAGE_SIZES[0]),pageSize=V21_PAGE_SIZES.includes(requestedPageSize)?requestedPageSize:V21_PAGE_SIZES[0],pages=Math.max(1,Math.ceil(list.length/pageSize)),page=clamp(Number(sp.get('page')||1),1,pages),shown=list.slice((page-1)*pageSize,page*pageSize);
...
function searchPageSizeSet(v){const sp=v21Params();sp.set('pageSize',String(v));sp.set('page','1');go(v21SearchUrl(sp));}
...
runAi=function(id){const bot=v21AvailableAiBots().find(b=>b.id===id||b.name===id)||V21_AI_BOTS.find(b=>b.id===id||b.name===id);if(!bot){toast('このAIボットは現在利用できません');return}
 showModal(`<div class="section-head"><div><h2>${esc(bot.name)}</h2><div class="small muted">基本プロンプト v${bot.promptVersion||1}</div></div><button class="icon-btn" onclick="closeModal()">×</button></div><div class="field"><label>テンプレート</label><div class="btn-row" id="aiTpls"><button class="chip" onclick="selectAiTemplate(this,'improve')">商品ページを改善</button><button class="chip" onclick="selectAiTemplate(this,'short')">短く要約</button><button class="chip" onclick="selectAiTemplate(this,'beginner')">初心者向け</button></div></div><div class="field"><label>対象商品</label><select id="aiProduct">${products.filter(p=>p.creatorId===currentUser()?.creatorId).concat(products.slice(0,2)).slice(0,5).map(p=>`<option value="${p.id}">${p.name}</option>`).join('')}</select></div><button class="btn" onclick="useCurrentProductForAi()">現在の商品情報を使う</button><div class="field"><label>入力</label><textarea id="aiInput"></textarea></div><button class="btn primary" id="aiRunBtn" onclick="executeV21Ai('${bot.id}')">実行（デモ）</button><div id="aiResult" class="notice" style="margin-top:12px">結果はここに表示されます。</div>`);
};
...
function executeV21Ai(botId){const bot=v21AvailableAiBots().find(b=>b.id===botId)||V21_AI_BOTS.find(b=>b.id===botId);if(!bot){toast('このAIボットは現在利用できません');return}const btn=$('#aiRunBtn');btn.disabled=true;busy('AIを実行しています…');setTimeout(()=>{done();btn.disabled=false;const box=$('#aiResult');if(!box)return;userData().lastAiBotRun={botId:bot.id,promptVersion:bot.promptVersion||1,input:$('#aiInput')?.value||'',date:now()};save();box.innerHTML=`<b>提案</b><p>${esc(bot.name)}の観点では、商品名・用途説明・タグの順で改善すると効果を確認しやすいです。</p><div class="small muted">使用プロンプト v${bot.promptVersion||1}</div><div class="btn-row"><button class="btn" onclick="toast('変更前後を比較表示しました（デモ）')">変更前後を比較</button><button class="btn primary" onclick="toast('商品下書きへ適用しました（デモ）');closeModal()">確認して適用</button><button class="btn" onclick="toast('直前の適用を元に戻しました（デモ）')">元に戻す</button></div>`;},700);}
...
adminAi=function(){db.admin.aiBots??=V21_AI_BOTS.map(x=>({...x,status:'公開',target:'creator',prompt:v21DefaultBotPrompt(x),promptVersion:1,updatedAt:now()}));let changed=false;db.admin.aiBots.forEach(b=>{if(!b.prompt){b.prompt=v21DefaultBotPrompt(b);changed=true}if(!b.promptVersion){b.promptVersion=1;changed=true}});if(changed)save();const bots=db.admin.aiBots;adminWorkbench('AIボット管理',`<div class="section-head admin-ai-head"><p>プロンプト、対象、公開状態を管理できます。AIの提案は自動適用されません。</p><button class="btn primary admin-ai-add-btn" onclick="adminAiModal()">＋ AIボット追加</button></div><div class="table-wrap"><table class="table"><tr><th>botId</th><th>名称</th><th>カテゴリ</th><th>対象</th><th>状態</th><th>版</th><th>操作</th></tr>${bots.map(b=>`<tr><td>${b.id}</td><td>${b.name}</td><td>${b.category}</td><td>${b.target}</td><td>${b.status}</td><td>v${b.promptVersion||1}</td><td><button class="btn" onclick="adminAiModal('${b.id}')">編集</button></td></tr>`).join('')}</table></div>`,'admin-ai')};
function adminAiModal(id){const existing=(db.admin.aiBots||[]).find(x=>x.id===id),b=existing||{id:uid('bot'),name:'',category:'商品制作',desc:'',target:'creator',status:'下書き'};b.prompt||=v21DefaultBotPrompt(b);showModal(`<div class="section-head"><div><h2>${id?'AIボット編集':'AIボット追加'}</h2>${id?`<div class="small muted">プロンプト v${b.promptVersion||1}</div>`:''}</div><button class="icon-btn" onclick="closeModal()">×</button></div><div class="field"><label>botId</label><input id="abId" value="${esc(b.id)}" ${id?'disabled':''}></div><div class="field"><label>名称</label><input id="abName" value="${esc(b.name)}"></div><div class="field"><label>カテゴリ</label><select id="abCat">${V21_AI_CATS.slice(1).map(x=>`<option ${b.category===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="field"><label>説明</label><textarea id="abDesc">${esc(b.desc)}</textarea></div><div class="field admin-ai-prompt-field"><label>AIへのプロンプト</label><p class="small muted">利用者の入力より前に適用する基本指示です。目的・出力形式・禁止事項を記載してください。</p><textarea id="abPrompt" rows="14">${esc(b.prompt)}</textarea></div><div class="field"><label>対象</label><select id="abTarget"><option ${b.target==='creator'?'selected':''}>creator</option><option ${b.target==='admin'?'selected':''}>admin</option></select></div><div class="field"><label>状態</label><select id="abStatus"><option ${b.status==='公開'?'selected':''}>公開</option><option ${b.status==='停止'?'selected':''}>停止</option><option ${b.status==='下書き'?'selected':''}>下書き</option></select></div><button class="btn primary full" onclick="saveAdminBot('${id||''}')">保存</button>`)}
function saveAdminBot(oldId){const existing=(db.admin.aiBots||[]).find(x=>x.id===oldId),v={id:oldId||$('#abId').value.trim(),name:$('#abName').value.trim(),category:$('#abCat').value,desc:$('#abDesc').value.trim(),prompt:$('#abPrompt').value.trim(),target:$('#abTarget').value,status:$('#abStatus').value};if(!v.id||!v.name){toast('botIdと名称は必須です');return}if(!/^[A-Za-z0-9_-]+$/.test(v.id)){toast('botIdは半角英数字・ハイフン・アンダースコアで入力してください');return}if(v.status==='公開'&&!v.prompt){toast('公開するAIボットにはプロンプトが必要です');return}const promptChanged=!!existing&&existing.prompt!==v.prompt;v.promptVersion=existing?Number(existing.promptVersion||1)+(promptChanged?1:0):1;v.updatedAt=now();db.admin.aiBots??=[];const i=db.admin.aiBots.findIndex(x=>x.id===oldId);if(i>=0)db.admin.aiBots[i]={...db.admin.aiBots[i],...v};else db.admin.aiBots.push(v);db.admin.audit??=[];db.admin.audit.unshift({date:now(),userId:currentUser().id,action:oldId?'AIボット編集':'AIボット追加',before:existing?`v${existing.promptVersion||1}`:'—',after:`${v.id} v${v.promptVersion}`,source:'AIボット管理'});save();closeModal();render();}
